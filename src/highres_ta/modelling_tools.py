...
def blank_map(projection=ccrs.PlateCarree(), lon_res=1.0, lat_res=1.0):
    ...
    gl = ax.gridlines(
        crs=projection, draw_labels=False, linewidth=0.5, color="gray", alpha=0.5, linestyle="--"
    )
...
def plot_actual_vs_predicted(
    y_true, y_pred, ax=None, fig=None, title="Actual vs predicted", label=""
):
...
def plot_residuals_vs_predicted(
    residuals, y_pred, ax=None, fig=None, title="Residuals vs Predicted", label=""
):
...
def plot_residuals_distribution(
    residuals, ax=None, fig=None, title="Residuals distribution", label=""
):
...
def plot_residual_vs_predictor(
    residuals, x_eval, feature_name, ax=None, fig=None, title="Residual vs predictor", label=""
):
...
class subModel:
    ...
    def simple_training(self, x_train, y_train, x_eval, y_eval, overwrite=False, export=False):
        ...
        self.predictions = pd.DataFrame(
            {"y_pred": y_pred, "y_eval": y_eval, "residuals": y_pred - y_eval}
        )
    ...
    def export_submodel(self):
        ...
        self.features_importance.to_csv(
            self.filepath / f"{self.name}_features_importance.csv", index=False
        )
    ...
    def plot_residuals_distribution(self, title="Residuals Distribution", ax=None, fig=None):
        plot_residuals_distribution(
            self.predictions["residuals"], title=title, label=self.name, ax=ax, fig=fig
        )
    ...
    def plot_residual_vs_predictor(
        self, x_eval, feature_name, title="Residuals vs predictor", ax=None, fig=None
    ):
        plot_residual_vs_predictor(
            self.predictions["residuals"],
            x_eval,
            feature_name,
            title=title,
            label=self.name,
    ...
    def plot_residuals_vs_time(self, validation_time, title="", ax=None, fig=None):
        plot_residuals_vs_time(
            self.predictions["residuals"], validation_time, title=title, ax=ax, fig=fig
        )
    ...
    def plot_residual_vs_all_predictors(self, x_eval):
        ...
        for i, feature_name in enumerate(self.features):
            self.plot_residual_vs_predictor(
                self.predictions["residuals"],
                x_eval,
                feature_name,
                title=feature_name,
...
class ModelWrapper:
    ...
    def cross_validation_run(self, cv_folds, overwrite=False, export=False):
        ...
        for fold in cv_folds:
            ...
            fold_model.simple_training(
                x_train_fold, y_train_fold, x_eval_fold, y_eval_fold, overwrite=True, export=export
            )
            ...
            fold_feat_importance_df = fold_model.features_importance.rename(
                columns={"importance": fold}
            )
            self.cv_features_importance = pd.concat(
                [self.cv_features_importance, fold_feat_importance_df], axis=1
            )
    ...
    def single_training(self, x_train, y_train, X_eval, y_eval, overwrite=False, export=False):
        ...
        self.single_training_model.simple_training(
            x_train, y_train, X_eval, y_eval, overwrite=overwrite, export=export
        )
    ...
    def export_cv_results(self):
        ...
        self.cv_features_importance.to_csv(
            self.filepath / f"{self.name}_cv_features_importance.csv"
        )
    ...
    def plot_cv_residual_vs_predictor(
        self, cv_fold_dict, feature_name, title="", ax=None, fig=None
    ):
    ...
    def plot_cv_residual_vs_all_predictors(self, cv_folds_dict):
        ...
        for i, feature_name in enumerate(self.features):
            for fold in self.cv_models_dict.keys():
                ...
                fold_model.plot_residual_vs_predictor(
                    x_eval=x_eval, feature_name=feature_name, ax=axes[i], fig=fig
                )
    ...
    def plot_cv_residuals_vs_time(self, cv_folds_dict):
        ...
        for i, fold in enumerate(self.cv_models_dict.keys()):
            ...
            fold_model.plot_residuals_vs_time(
                validation_time, title=f"Fold: {fold}", ax=axes[i], fig=fig
            )
...
class Experiment:
    ...
    def add_an_existing_model(self, trained_model_wrapper: ModelWrapper):
        ...
        if trained_model_wrapper.name in self.models_dict:
            raise ValueError(
                f"Model with name {trained_model_wrapper.name} already exists in the experiment."
            )
    ...
    def update_summary_tables(self):
        ...
        self.average_features_importance.to_csv(
            self.filepath / f"{self.name}_average_cv_features_importance.csv"
        )
    ...
    def compare_cv_eval_scores_boxplot(self, ncols=2):
        ...
        for i, metric in enumerate(metrics):
            models_scores = [
                self.models_dict[model_name].cv_eval_scores.loc[metric]
    ...
    def compare_cv_residual_vs_predictor(self, feature_name, ncols=2):
        ...
        for i, model_name in enumerate(self.models_dict.keys()):
            model = self.models_dict[model_name]
            model.plot_cv_residual_vs_predictor(
                cv_fold_dict=self.cv_folds, feature_name=feature_name, ax=axes[i], fig=fig
            )
            axes[i].set_title(model_name)
