...
def keep_predictors(df):
    ...
    drop_cols = (
    ['talk_gp','talk_gp_normalized', 'year_gp', 'expocode_gp', 'salinity_bin', 'tco2_gp', 'fco2_gp', 'silicate_gp', 'phtsinsitutp_gp', 'oxygen_gp', 'nitrate_gp', 'nitrite_gp', 'phosphate_gp',  'aou_gp', 'salinity_soda','temp_soda'] 
...
def traintest_salinity_expocode_split(source_df):
    ...
    splits = splitter.split(X= source_df, y = source_df['salinity_bin'], groups= source_df["expocode_gp"])
...
def cv_salinity_expocode_split(source_train_df: pd.DataFrame, n_splits = 5, normalized_y = False, random_seed = 42, plot_cv_splits = True):
    ...
    cv_splits = list(cv_splitter.split(source_train_df, source_train_df['salinity_bin'], groups=source_train_df["expocode_gp"]))

    # Extract the first validation split from cv_splits
    cv_folds = dict()
    
    x_training = keep_predictors(source_train_df)
    y_training = source_train_df['talk_gp_normalized'] if normalized_y else source_train_df['talk_gp']
